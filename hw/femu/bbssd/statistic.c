...
void flush_to_file(struct statistic *statistics, int num, int sec)
{
    ...
    sprintf(buff+strlen(buff), "[%4d] ", sec);
    for( int i = 0; i < num; i++ ){
        struct time_unit *u = statistics[i].cur;
        tot_us_read+=u->us_read;
        tot_us_write+=u->us_write;
        tot_gc_write+=u->gc_write;
        sprintf(buff+strlen(buff),"%5ld %5ld %5ld | ", u->us_read, u->us_write, u->gc_write);
    }
    sprintf(buff+strlen(buff),"%5ld %5ld %5ld", tot_us_read, tot_us_write, tot_gc_write);
